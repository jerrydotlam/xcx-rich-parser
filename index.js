  ...
  node.forEach(function (item) {
    switch (item.type) {
      case 'tag':
        ...
        var attributes = item.attribs;
        console.log(_getPrefix(level) + name);
        // 小程序暂时不支持`pre`标签
        if (name === 'pre') {
          name = 'div';
        }
        if (TAG_LIST.indexOf(name) >= 0) {
          if (children && children.length > 0) {
            children = _parse(children, level + 1);
          }
          var _tmp = {
            // type: 'node', // 可以不写
            name: name,
            attrs: {
              class: 'rich-text-' + item.name
            },
            children: children
          }
          if (name === 'img') {
            !!attributes.alt && (_tmp.attrs.alt = attributes.alt)
            !!attributes.src && (_tmp.attrs.src = attributes.src)
            !!attributes.src && (_tmp.attrs['data-src'] = attributes.src)
            console.log('===========================')
            console.log(_tmp.attrs, attributes)
            console.log('===========================')
          } else if (name === 'col' || name === 'colgroup') {
            !!attributes.span && (_tmp.attrs.span = attributes.span)
          } else if (name === 'td' || name === 'th') {
            !!attributes.colspan && (_tmp.attrs.colspan = attributes.colspan)
            !!attributes.rowspan && (_tmp.attrs.rowspan = attributes.rowspan)
          }
          result.push(_tmp);
        }
        break;
      case 'text':
        ...
        data = data.trim()
        if (data) {
          console.log(_getPrefix(level) + data)
          result.push({
            type: 'text',
            text: data
          })
